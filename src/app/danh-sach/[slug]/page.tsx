...
export default function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  const { data, loading } = useFetchData<MovieType>(`/danh-sach/${params.slug}?page=${page}`);
  ...
  return (
    <div className='container py-[5rem] md:py-[7rem]'>
      <BreadcrumbComponent title={data?.data.breadCrumb[0].name} subtitle='Danh sách phim' currentPage={data?.data.breadCrumb[1].name} />
